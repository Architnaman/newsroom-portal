...
export default function Chatbot() {
  ...
  async function getDBContext() {
    if (role === "editor") {
      ...
      const { data: leaves } = await supabase.from("leave_requests").select("id, reporter_id, leave_date, leave_type, status, notes").eq("status", "pending")
      const { data: assignments } = await supabase.from("assignments").select("story_id, reporter_id").eq("is_active", true)
      return { stories, reporters, leaves, assignments }
    } else {
      ...
      return { myAssignments, leaves, availability }
    }
  }
  ...
  async function executeAction(action: any) {
    try {
      ...
      if (action.type === "assign_story") {
        ...
        await supabase.from("assignments").insert({ story_id: action.story_id, reporter_id: action.reporter_id, is_active: true })
...
2. Assign story: {"action": {"type": "assign_story", "story_id": "...", "reporter_id": "..."}}
3. Approve leave: {"action": {"type": "approve_leave", "leave_id": "..."}}
4. Reject leave: {"action": {"type": "reject_leave", "leave_id": "...", "reason": "..."}}
5. Publish story: {"action": {"type": "publish_story", "story_id": "...", "feedback": "optional"}}

RULES:
- If fields are missing ask for them one by one in a friendly way
- Deadline must be within current week (${today} to Sunday)
- Always confirm before executing
- When executing an action return ONLY valid JSON: {"action": {...}, "message": "..."}
      ...
      {open && (
        <div style={{ width: "380px", height: "520px", background: "#0d0d14", border: "1px solid rgba(255,180,0,0.3)", borderRadius: "12px", display: "flex", flexDirection: "column", marginBottom: "12px", boxShadow: "0 20px 60px rgba(0,0,0,0.5)" }}>
          ...
          <div style={{ padding: "16px 20px", borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <button onClick={() => setOpen(false)} style={{ background: "none", border: "none", color: "#555", fontSize: "18px", cursor: "pointer" }}>×</button>
          </div>
