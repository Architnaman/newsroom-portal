...
export default function CalendarPage() {
  ...
  const { t } = useTheme()
  const { formatDate, weekStartDay } = useDateFormat()
  ...
  async function load(targetId?: string) {
    setLoading(true)
    if (targetId) {
      const { data: leaveData } = await supabase.from('leave_requests').select('*').eq('reporter_id', targetId).order('leave_date')
      setLeaves(leaveData || [])
      const { data: availData } = await supabase.from('availability').select('*').eq('reporter_id', targetId)
      setAvailability(availData || [])
      const { data: assignData } = await supabase.from('assignments').select(`id, assigned_at, stories (id, headline, urgency, status, deadline, category, complexity, priority)`).eq('reporter_id', targetId).eq('is_active', true)
      setAssignments(assignData || [])
    } else {
      setLeaves([]); setAvailability([]); setAssignments([])
    }
    if (role === 'editor') {
      const { data: reporterData } = await supabase.from('reporters').select('id, name, email, beats').eq('status', 'active')
  ...
  function handleDayClick(dateStr: string) {
    ...
    if (role === 'reporter') {
      ...
      if (status.holiday || status.leave || status.storiesDeadline.length > 0 || status.storiesActive.length > 0) {
        setDayDetailModal({ dateStr, status }); return
      }
      setSelectedDate(dateStr); setLeaveModal(true)
    } else {
  ...
  async function submitLeave() {
    ...
    await supabase.from('leave_requests').insert({
      reporter_id: reporterId, leave_date: selectedDate,
      leave_type: leaveForm.leave_type,
      is_immediate: leaveForm.leave_type === 'emergency' || leaveForm.leave_type === 'sick',
      notes: leaveForm.notes, status: 'pending'
    })
    setSubmitting(false); setLeaveModal(false); setSelectedDate(null)
  ...
  const legend = [
    ...
    { color: t.danger, border: t.dangerBorder, label: 'Holiday' },
    ...
    <div style={{ minHeight: '100vh', background: t.bgPage, fontFamily: '"Inter", "DM Mono", "Courier New", monospace', color: t.textPrimary }}>
      ...
      <main role="main" style={{
        ...
        <div style={{
          ...
          {role === 'editor' && (
            <select value={selectedReporter} onChange={e => setSelectedReporter(e.target.value)} aria-label="Select reporter"
              style={{ padding: '10px 16px', background: t.bgCard, border: `1px solid ${t.accentBorder}`, borderRadius: '8px', color: t.accent, fontSize: '13px', outline: 'none', fontFamily: 'inherit', cursor: 'pointer', fontWeight: '600', boxShadow: t.shadowCard, width: isMobile ? '100%' : 'auto', minHeight: '44px' }}>
              {reporters.map(r => <option key={r.id} value={r.id} style={{ background: t.bgCard }}>{r.name}</option>)}
            </select>
          )}
        </div>
      ...
      {leaveModal && selectedDate && (
        <div role="dialog" aria-modal="true"
          ...
          <div style={{ background: t.bgCard, border: `1px solid ${t.accentBorder}`, borderRadius: isMobile ? '14px 14px 0 0' : '12px', width: '100%', maxWidth: isMobile ? '100%' : '420px', margin: isMobile ? '0' : '24px', padding: isMobile ? '20px 16px' : '28px', fontFamily: 'inherit', boxShadow: t.shadow }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '16px', alignItems: 'center' }}>
              <h2 style={{ color: t.textPrimary, margin: 0, fontSize: isMobile ? '16px' : '18px', fontWeight: '700' }}>Apply for Leave</h2>
              <button onClick={() => { setLeaveModal(false); setSelectedDate(null) }} style={{ background: 'none', border: 'none', color: t.textMuted, fontSize: '22px', cursor: 'pointer', minWidth: '44px', minHeight: '44px' }}>x</button>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <div>
                ...
                <textarea value={leaveForm.notes} onChange={e => setLeaveForm(p => ({ ...p, notes: e.target.value }))} rows={3} placeholder="Reason for leave..." style={inputStyle} />
              </div>
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                <button onClick={() => { setLeaveModal(false); setSelectedDate(null) }} style={{ flex: 1, padding: '12px', background: 'transparent', border: `1px solid ${t.borderCard}`, borderRadius: '8px', color: t.textMuted, fontSize: '13px', cursor: 'pointer', fontFamily: 'inherit', minHeight: '48px' }}>CANCEL</button>
                <button onClick={submitLeave} disabled={submitting} style={{ flex: 2, padding: '12px', background: submitting ? t.textMuted : t.accent, border: 'none', borderRadius: '8px', color: t.accentText, fontSize: '13px', fontWeight: '700', cursor: submitting ? 'not-allowed' : 'pointer', fontFamily: 'inherit', opacity: submitting ? 0.6 : 1, minHeight: '48px' }}>
      ...
      {dayDetailModal && (
        <div role="dialog" aria-modal="true"
          ...
          <div style={{ background: t.bgCard, border: `1px solid ${t.borderCard}`, borderRadius: isMobile ? '14px 14px 0 0' : '12px', width: '100%', maxWidth: isMobile ? '100%' : '520px', margin: isMobile ? '0' : '24px', padding: isMobile ? '20px 16px' : '28px', fontFamily: 'inherit', boxShadow: t.shadow, maxHeight: isMobile ? '85vh' : '88vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '20px', alignItems: 'flex-start' }}>
              <div>
                <p style={{ color: t.textMuted, fontSize: '11px', fontWeight: '600', letterSpacing: '0.5px', margin: '0 0 4px' }}>DATE</p>
                <h2 style={{ color: t.textPrimary, margin: 0, fontSize: isMobile ? '16px' : '20px', fontWeight: '700' }}>{dayDetailModal.dateStr}</h2>
              </div>
            ...
            {(() => {
              ...
              return (
                <div style={{ marginBottom: '14px' }}>
                  <p style={{ color: t.textMuted, fontSize: '11px', fontWeight: '700', letterSpacing: '0.5px', margin: '0 0 10px' }}>STORIES ON THIS DAY — {allS.length}</p>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                    {allS.map((story: any) => {
                      ...
                      const daysLeft = Math.ceil((new Date(story.deadline).getTime() - new Date(today).getTime()) / 86400000)
                      return (
                        <div key={story.id} style={{ padding: isMobile ? '12px' : '14px 16px', borderRadius: '8px', border: `1px solid ${uc}40`, background: `${uc}08`, borderLeft: `4px solid ${uc}` }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px', flexWrap: 'wrap' }}>
                            <span style={{ padding: '3px 8px', borderRadius: '4px', fontSize: '10px', fontWeight: '700', background: `${uc}20`, color: uc, border: `1px solid ${uc}40` }}>{story.urgency?.toUpperCase()}</span>
                            <span style={{ padding: '3px 8px', borderRadius: '4px', fontSize: '10px', fontWeight: '600', background: `${statusColor[story.status]}15`, color: statusColor[story.status] }}>{story.status?.replace('_', ' ').toUpperCase()}</span>
                            {isDeadline && <span style={{ padding: '3px 8px', borderRadius: '4px', fontSize: '10px', fontWeight: '700', background: t.dangerBg, color: t.danger, border: `1px solid ${t.dangerBorder}` }}>DEADLINE TODAY</span>}
                          </div>
                          <p style={{ color: t.textPrimary, fontSize: isMobile ? '13px' : '15px', fontWeight: '700', margin: '0 0 10px', lineHeight: 1.4 }}>{story.headline}</p>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap', marginBottom: '8px' }}>
                            <div style={{ padding: '5px 10px', background: t.bgCard, border: `1px solid ${t.borderCard}`, borderRadius: '6px' }}>
                              <span style={{ color: t.textMuted, fontSize: '10px', fontWeight: '600', display: 'block', marginBottom: '1px' }}>START</span>
                              <span style={{ color: t.textSecondary, fontSize: '12px', fontWeight: '700' }}>{story.assigned_at}</span>
                            </div>
                            <span style={{ color: t.textMuted, fontSize: '14px', fontWeight: '700' }}>→</span>
                          ...
                          <div style={{ display: 'flex', gap: '12px' }}>
                            ...
                            <span style={{ color: t.textMuted, fontSize: '12px' }}>Priority: <span style={{ color: t.textSecondary, fontWeight: '600' }}>P{story.priority}</span></span>
                          </div>
                        </div>
                      )
                    })}
            ...
            {dayDetailModal.status.leave && (
              <div style={{ padding: '14px 16px', background: dayDetailModal.status.leave.status === 'acknowledged' ? t.warningBg : dayDetailModal.status.leave.status === 'rejected' ? t.dangerBg : t.accentBg, border: `1px solid ${dayDetailModal.status.leave.status === 'acknowledged' ? t.warningBorder : dayDetailModal.status.leave.status === 'rejected' ? t.dangerBorder : t.accentBorder}`, borderRadius: '8px', marginBottom: '12px' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ color: ltc[dayDetailModal.status.leave.leave_type], fontSize: '13px', fontWeight: '700' }}>{dayDetailModal.status.leave.leave_type?.toUpperCase()}</span>
                  <span style={{ padding: '3px 10px', borderRadius: '4px', fontSize: '11px', fontWeight: '700', background: dayDetailModal.status.leave.status === 'acknowledged' ? t.successBg : dayDetailModal.status.leave.status === 'rejected' ? t.dangerBg : t.accentBg, color: dayDetailModal.status.leave.status === 'acknowledged' ? t.success : dayDetailModal.status.leave.status === 'rejected' ? t.danger : t.accent, border: `1px solid ${dayDetailModal.status.leave.status === 'acknowledged' ? t.successBorder : dayDetailModal.status.leave.status === 'rejected' ? t.dangerBorder : t.accentBorder}` }}>{dayDetailModal.status.leave.status?.toUpperCase()}</span>
                </div>
              </div>
            )}
            ...
            {!dayDetailModal.status.holiday && !dayDetailModal.status.leave && (
              <div style={{ padding: '14px 16px', background: dayDetailModal.status.isWeekend ? t.bgPage : dayDetailModal.status.isAvailable ? t.successBg : t.dangerBg, border: `1px solid ${dayDetailModal.status.isWeekend ? t.borderCard : dayDetailModal.status.isAvailable ? t.successBorder : t.dangerBorder}`, borderRadius: '8px', marginBottom: '12px' }}>
                ...
                <p style={{ color: dayDetailModal.status.isWeekend ? t.textDisabled : dayDetailModal.status.isAvailable ? t.success : t.danger, fontSize: '15px', fontWeight: '700', margin: 0 }}>
