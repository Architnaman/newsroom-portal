    ...
    <div style={{ minHeight: '100vh', background: t.bgPage, fontFamily: '"Inter", "DM Mono", sans-serif', color: t.textPrimary }}>
      ...
      <main style={{ padding: '32px 24px', maxWidth: '860px', margin: '0 auto' }}>
        ...
        <div style={{ marginBottom: '32px' }}>
          ...
          <h1 style={{ color: t.textPrimary, margin: '0 0 6px', fontSize: '24px', fontWeight: '700' }}>System Settings</h1>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px', marginBottom: '28px' }}>
          {[
            ...
            <div key={stat.label} style={{ padding: '20px', background: stat.bg, border: `1px solid ${stat.border}`, borderRadius: '10px', textAlign: 'center', boxShadow: t.shadowCard }}>
              <div style={{ color: stat.color, fontSize: '36px', fontWeight: '800', lineHeight: 1, marginBottom: '8px' }}>{stat.value}</div>
              <div style={{ color: t.textMuted, fontSize: '12px', fontWeight: '600', letterSpacing: '0.5px' }}>{stat.label.toUpperCase()}</div>
            </div>
          ))}
        </div>
        ...
        <div style={{ ...cardStyle, border: `1px solid ${t.accentBorder}`, background: t.accentBg }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
            <div style={{ padding: '16px', background: t.bgCard, borderRadius: '8px', border: `1px solid ${t.borderCard}` }}>
              <p style={{ color: t.textMuted, fontSize: '11px', fontWeight: '600', margin: '0 0 8px', letterSpacing: '0.5px' }}>
                ALL DATES WILL LOOK LIKE
              </p>
              <p style={{ color: t.textPrimary, fontSize: '20px', fontWeight: '800', margin: '0 0 4px' }}>
                {formatDate(today)}
              </p>
              <p style={{ color: t.textMuted, fontSize: '11px', margin: 0 }}>
                Applied to deadlines, leave dates, assigned dates everywhere
              </p>
            </div>
            <div style={{ padding: '16px', background: t.bgCard, borderRadius: '8px', border: `1px solid ${t.borderCard}` }}>
              <p style={{ color: t.textMuted, fontSize: '11px', fontWeight: '600', margin: '0 0 8px', letterSpacing: '0.5px' }}>
              ...
              <p style={{ color: t.success, fontSize: '20px', fontWeight: '800', margin: '0 0 4px' }}>
                {pendingWeekStart === 'monday' ? 'Monday' : 'Sunday'}
              </p>
              <p style={{ color: t.textMuted, fontSize: '11px', margin: 0 }}>
                Affects calendar, roster and availability pages
              </p>
            </div>
          </div>
        </div>

        {/* Date Format — SINGLE picker */}
        <div style={cardStyle}>
          <h2 style={{ color: t.textPrimary, margin: '0 0 6px', fontSize: '16px', fontWeight: '700' }}>
            Date Format
          </h2>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '10px' }}>
            {DATE_FORMATS.map(fmt => (
              <button key={fmt.value} onClick={() => setPendingDateFormat(fmt.value)}
                ...
                <div style={{ color: pendingDateFormat === fmt.value ? t.accent : t.textPrimary, fontSize: '14px', fontWeight: '700', marginBottom: '6px' }}>
                  {fmt.value}
                </div>
                <div style={{ color: t.textMuted, fontSize: '13px', marginBottom: '6px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
            {[
              ...
              <button key={opt.value} onClick={() => setPendingWeekStart(opt.value as any)}
                style={{
                  padding: '18px 20px', borderRadius: '8px', cursor: 'pointer',
                  fontFamily: 'inherit', textAlign: 'left' as const,
                ...
                <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
                  {opt.days.map((d, i) => (
                    <span key={d} style={{
                      ...
                      background: i < 5 ? t.accentBg : t.bgCard,
                      color: i < 5 ? t.accent : t.textMuted,
                      border: `1px solid ${i < 5 ? t.accentBorder : t.borderCard}`
                    }}>{d}</span>
                  ))}
                </div>
              </button>
        ...
        <div style={{ display: 'flex', gap: '12px', alignItems: 'center', justifyContent: 'flex-end' }}>
          {saved && (
            <div style={{ padding: '10px 20px', background: t.successBg, border: `1px solid ${t.successBorder}`, borderRadius: '8px' }}>
              <span style={{ color: t.success, fontSize: '13px', fontWeight: '700' }}>
                ✓ Saved! All portals updated instantly.
              </span>
            </div>
          )}
          <button onClick={saveSettings} disabled={saving}
            style={{
              padding: '14px 40px', background: saving ? t.textMuted : t.accent,
